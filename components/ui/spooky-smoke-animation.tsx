...
export function SmokeBackground({ smokeColor = '#808080', animateColors = false }: SmokeBackgroundProps) {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      if (animateColors) {
        const hue = (now * 0.018) % 360
        renderer.updateColor(hslToRgb(hue, 85, 52))
      }
      renderer.render(now)
